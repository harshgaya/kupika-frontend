...
//         return;
//       }
//       router.push(`/checkout?checkout=${res.checkoutId}`);
//     } catch (err) {
...
import { getGuestId, resolveProduct, HERO_TRUST_PILLS } from "./data";
import {
  WhyChoose,
  Ingredients,
  HowToUse,
...
// ── Small inline helpers used by the hero ─────────────────────────────────────
...
function QtyStepper({ qty, onIncrease, onDecrease, square }) {
  const wrap = square
    ? "rounded-xl border border-stone-200 bg-stone-50"
    : "rounded-full border border-stone-200 bg-white shadow-sm";
  const btn = square
    ? "w-9 h-10"
    : "w-9 h-9 hover:text-[#1b4332] transition-colors";
  const count = square ? "w-7" : "w-8";
  return (
    <div className={`flex items-center ${wrap}`}>
      <button
        onClick={onDecrease}
        className={`${btn} flex items-center justify-center text-stone-600 font-bold text-lg`}
      >
        −
      </button>
      <span className={`${count} text-center text-sm font-bold text-stone-800`}>
        {qty}
      </span>
      <button
        onClick={onIncrease}
...
function BuyButton({ loading, onClick, total, className }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className={`flex items-center justify-center gap-2 text-white transition-all duration-200 ${
        loading
          ? "bg-stone-400 cursor-not-allowed"
          : "bg-[#1b4332] active:scale-[0.98]"
      } ${className}`}
    >
      {loading ? (
        <>
...
// Desktop thumbnail gallery (own internal active state).
...
export default function ProductDetails({ product: rawProduct }) {
  ...
  // Merge raw product with fallbacks once. Everything reads from this.
  const product = useMemo(() => resolveProduct(rawProduct), [rawProduct]);
  ...
  const handleBuy = async () => {
    ...
    try {
      setLoading(true);
      const res = await createCheckout({ productId: rawProduct._id, quantity: qty });
      if (!res) {
        alert("Failed to add product. Please try again.");
        return;
      }
  ...
  const { badges, rating, reviewCount, discount, descriptionPoints } = product;
  const images = product.gallery_images || [];
  const buyTotal = product.selling_price * qty;
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="flex justify-center">
            <div className="w-full max-w-lg">
              ...
              <DesktopGallery images={images} title={product.title} />
            </div>
          </div>
          ...
          <div className="pb-24 lg:pb-0">
            ...
            <BuyButton
              ...
              onClick={handleBuy}
              total={buyTotal}
              className="hidden lg:flex w-full rounded-2xl py-4 text-base font-bold shadow-lg shadow-emerald-900/20 hover:bg-emerald-700"
            />

            {/* Trust pills */}
            <div className="hidden lg:flex mt-4 flex-wrap gap-3 text-xs text-stone-500 font-medium">
              {HERO_TRUST_PILLS.map((t) => (
                <span
                  key={t}
                  className="flex items-center gap-1 bg-white border border-stone-100 rounded-full px-3 py-1.5 shadow-sm"
                >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-stone-200 px-4 py-3 shadow-2xl shadow-black/20">
        <div className="flex items-center gap-3 max-w-lg mx-auto">
          ...
          <BuyButton
            ...
            onClick={handleBuy}
            total={buyTotal}
            className="flex-1 rounded-xl py-3.5 text-sm font-bold"
          />
        </div>
      </div>
