...
export const isIndianMobile = (v) =>
  /^[6-9]\d{9}$/.test(clean(v).replace(/\D/g, ""));
...
export function validateAddress(a = {}) {
  ...
  if (!isIndianMobile(a.phone))
    errors.phone = "Enter a valid 10-digit mobile number.";
  if (clean(a.street_address || a.address).length < 5)
    errors.street_address = "Enter house, street and locality.";
...
export function validateOrderInput(b = {}) {
  const errors = validateAddress({
    ...b.address,
    ...b,
    name: b.customer_name || b.address?.name,
    phone: b.mobile_number || b.phone || b.address?.phone,
    street_address: b.address?.street_address || b.street_address,
  ...
  if (!["cod", "prepaid_500", "prepaid_full"].includes(plan))
    errors.payment_plan = "Select a valid payment option.";
  return errors;
}
export const firstError = (errors) =>
  Object.values(errors)[0] || "Invalid form data.";
